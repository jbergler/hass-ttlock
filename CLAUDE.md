...
This runs lint, type-check, and the test suite, in that order. Each step is also runnable on its own:
...
- `script/test` — `pytest`, extra args are passed through (e.g. `script/test -k coordinator`)

Pytest is configured with `filterwarnings = ["error"]` and `--strict-markers` — an unaddressed deprecation warning fails the suite, not just prints. If a warning legitimately originates from a dependency (not this integration's own code), add a narrowly-scoped `ignore::` entry to `[tool.pytest.ini_options].filterwarnings` in `pyproject.toml` rather than silencing it elsewhere.

Type checking uses [`ty`](https://github.com/astral-sh/ty) (Astral's type checker), not mypy or pyright. Where `ty` flags a genuine tool/stub-precision limitation rather than a real bug (e.g. voluptuous's `vol.All` as a schema, HA's abstract `OAuth2Implementation` typing, dict/list generic invariance against HA's recursive `JsonValueType` alias), the fix is a narrow `# ty: ignore[rule] - reason` comment on that line, not a broader suppression. Before reaching for `ty: ignore`, check whether the diagnostic is actually a real bug — several genuine bugs (a stale import path, a `None`-handling gap that could crash diagnostics, a service response returning raw `datetime` objects instead of ISO strings) were found this way during the mypy→ty migration.
...
- `webhook.py` — receives TTLock's push notifications; this is why lock state updates are near-real-time rather than poll-only
